...
export class NLPQueryEngine {
  ...
  async processNaturalLanguageQuery(query: string, userId?: string): Promise<QueryResult> {
    ...
    try {
      ...
      if (geminiResponse.sqlQuery && geminiResponse.sqlQuery !== 'NONE') {
        try {
          ...
          data = result.rows;
          logger.info(`SQL executed successfully, returned ${data.length} rows`);
        } catch (sqlError) {
          logger.error('SQL execution error:', sqlError);
          // Try to fix the SQL with Gemini
          const fixedQuery = await this.fixSQLWithGemini(sqlQuery, sqlError instanceof Error ? sqlError.message : 'Unknown error');
          if (fixedQuery) {
            try {
              const retryResult = await this.pool.query(fixedQuery);
              data = retryResult.rows;
              sqlQuery = fixedQuery;
              logger.info(`Fixed SQL executed successfully, returned ${data.length} rows`);
            } catch (retryError) {
              logger.error('Fixed SQL also failed:', retryError);
            }
          }
        }
      }
  ...
  private async processWithGemini(query: string, schemaContext: string): Promise<any> {
    const prompt = `
...
`;
  ...
  private async fixSQLWithGemini(brokenSQL: string, errorMessage: string): Promise<string | null> {
    const prompt = `
...
`;
    ...
    try {
      ...
      logger.error('Error fixing SQL with Gemini:', error);
      return null;
    }
  }

  private async generateInsights(originalQuery: string, data: any[], interpretation: string): Promise<string[]> {
...
store_info:
  ...
  private async storeQueryHistory(historyData: any): Promise<void> {
    try {
  ...
  async getQueryHistory(userId?: string, limit: number = 10): Promise<any[]> {
    try {
      const query = userId 
