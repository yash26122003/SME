...
export class WalmartDataLoader {
  private pool: Pool;
  ...
  async loadWalmartSalesData(): Promise<{ success: boolean; recordsLoaded: number; errors: string[] }> {
    const csvFilePath = path.join(process.cwd(), 'Walmart_Sales.csv');
    const errors: string[] = [];
    let recordsLoaded = 0;
    ...
    try {
      ...
      logger.info('Cleared existing Walmart sales data');

      // Read and process CSV data
      const records: WalmartSalesRow[] = [];

      return new Promise((resolve) => {
        fs.createReadStream(csvFilePath)
          .pipe(csv())
          .on('data', (row: WalmartSalesRow) => {
            records.push(row);
          })
          .on('end', async () => {
          ...
          .on('error', (error) => {
            ...
            errors.push(`CSV reading error: ${error instanceof Error ? error.message : 'Unknown error'}`);
            resolve({ success: false, recordsLoaded, errors });
          });
      });

    } catch (error) {
      logger.error('Error in data loading process:', error);
      return { success: false, recordsLoaded, errors: [error instanceof Error ? error.message : 'Unknown error'] };
    }
  }
  ...
  private async initializeTables(): Promise<void> {
    ...
    for (const statement of statements) {
      try {
        ...
        logger.warn(`Schema statement warning: ${error instanceof Error ? error.message : 'Unknown error'}`);
      }
    }
  }

  private async insertBatch(records: WalmartSalesRow[]): Promise<void> {
  ...
  async updateAnalyticsCache(): Promise<void> {
    try {
      ...
      for (const { key, query } of queries) {
        ...
        logger.info(`Updated analytics cache for: ${key}`);
      }
