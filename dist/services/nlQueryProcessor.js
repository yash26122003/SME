...
exports.nlQueryProcessor = exports.NaturalLanguageQueryProcessor = void 0;
...
class NaturalLanguageQueryProcessor {
    ...
    async parseIntent(query, context) {
        ...
        try {
            ...
            const intent = intentMatch ? intentMatch[1].toUpperCase() : query_1.QueryIntent.SUMMARY;
            logger_js_1.logger.info(`Parsed query intent: ${intent} for query: "${query}"`);
            return intent;
        }
        catch (error) {
            logger_js_1.logger.error('Error parsing query intent:', error);
            return query_1.QueryIntent.SUMMARY; // Fallback
        }
    }
    /**
     * Extracts entities from natural language query
     */
    async extractEntities(query, context) {
        ...
        try {
            ...
            logger_js_1.logger.error('Error extracting entities:', error);
            return [];
        }
    }
    /**
     ...
     */
    async generateSQL(processedQuery, context) {
        ...
        try {
            ...
            logger_js_1.logger.info(`Generated SQL: ${sql}`);
            return sql;
        }
        catch (error) {
            logger_js_1.logger.error('Error generating SQL:', error);
     ...
     */
    async validateQuery(sqlQuery, context) {
        const errors = [];
        const warnings = [];
        ...
        for (const pattern of dangerousPatterns) {
            if (pattern.test(sqlQuery)) {
                errors.push({
                    type: query_1.ErrorType.SYNTAX_ERROR,
        ...
        if (complexity === query_1.QueryComplexity.VERY_HIGH) {
            warnings.push({
                type: query_1.WarningType.PERFORMANCE,
     ...
     */
    async explainQuery(processedQuery) {
        const prompt = `
    ...
    `;
        try {
            ...
            logger_js_1.logger.error('Error explaining query:', error);
     ...
     */
    async generateSuggestions(partialQuery, context) {
    ...
    buildEntityExtractionPrompt(query, context) {
    ...
    buildSQLGenerationPrompt(processedQuery, context) {
        return `
    ...
    buildSuggestionsPrompt(partialQuery, context) {
        return `
    ...
    parseEntitiesFromResponse(response, query) {
        const entities = [];
    ...
    parseProcessedQueryResponse(response, originalQuery, context) {
        try {
            ...
            logger_js_1.logger.error('Error parsing processed query response:', error);
            return this.createFallbackProcessedQuery(originalQuery);
        }
    }
    mapToProcessedQuery(parsed, originalQuery) {
        return {
            intent: parsed.intent || query_1.QueryIntent.SUMMARY,
            entities: parsed.entities || [],
            timeframe: parsed.timeframe || { type: query_1.TimeframeType.RELATIVE, relative: query_1.RelativeTimeframe.LAST_30_DAYS },
    ...
    manualParseResponse(response, originalQuery) {
        // Simple manual parsing as fallback
        const lines = response.toLowerCase();
        let intent = query_1.QueryIntent.SUMMARY;
        if (lines.includes('trend') || lines.includes('over time'))
            intent = query_1.QueryIntent.TREND_ANALYSIS;
        else if (lines.includes('compare') || lines.includes('vs'))
            intent = query_1.QueryIntent.COMPARISON;
        else if (lines.includes('total') || lines.includes('sum'))
            intent = query_1.QueryIntent.AGGREGATION;
        return {
            intent,
            entities: [],
            timeframe: { type: query_1.TimeframeType.RELATIVE, relative: query_1.RelativeTimeframe.LAST_30_DAYS },
            metrics: [],
            filters: [],
            groupBy: [],
            orderBy: [],
    ...
    createFallbackProcessedQuery(originalQuery) {
        return {
            intent: query_1.QueryIntent.SUMMARY,
            entities: [],
            timeframe: { type: query_1.TimeframeType.RELATIVE, relative: query_1.RelativeTimeframe.LAST_30_DAYS },
            metrics: [],
            filters: [],
    ...
    estimateQueryComplexity(sqlQuery) {
        const query = sqlQuery.toLowerCase();
        let complexity = 0;
        // Count complexity factors
        if (query.includes('join'))
            complexity += 2;
        if (query.includes('subquery') || query.includes('select') && query.split('select').length > 2)
            complexity += 3;
        ...
        if (query.includes('window') || query.includes('over('))
            complexity += 2;
        if (complexity <= 2)
            return query_1.QueryComplexity.LOW;
        if (complexity <= 5)
            return query_1.QueryComplexity.MEDIUM;
        if (complexity <= 8)
            return query_1.QueryComplexity.HIGH;
        return query_1.QueryComplexity.VERY_HIGH;
    }
    estimateResourceUsage(sqlQuery, complexity) {
        const baseTime = 100; // milliseconds
        const baseCpu = 1;
        const baseMemory = 64; // MB
        const baseIO = 5;
        const multipliers = {
            [query_1.QueryComplexity.LOW]: 1,
            [query_1.QueryComplexity.MEDIUM]: 2,
            [query_1.QueryComplexity.HIGH]: 4,
            [query_1.QueryComplexity.VERY_HIGH]: 8
        };
    ...
    estimateRowCount(sqlQuery) {
        ...
        if (query.includes('where') && query.includes('timestamp'))
            return 1000;
        if (query.includes('group by'))
            return 50;
...
exports.NaturalLanguageQueryProcessor = NaturalLanguageQueryProcessor;
// Export singleton instance
exports.nlQueryProcessor = new NaturalLanguageQueryProcessor();
//# sourceMappingURL=nlQueryProcessor.js.map